...
@section('content')
@php $total = 0; @endphp <!-- total selalu ada -->
...
<div class="container mt-4">
    ...
    @else
        ...
        <!-- Checkout Form -->
        <form action="{{ route('admin.cart.checkout') }}" method="POST" id="checkoutForm">
            @csrf
        ...
        <!-- Save Draft Form -->
        <form action="{{ route('admin.cart.saveDraft') }}" method="POST" class="mt-2">
            @csrf
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    cartTable.addEventListener('click', function(e){
        if(e.target.classList.contains('btn-increase') || e.target.classList.contains('btn-decrease')) {
            ...
            if(e.target.classList.contains('btn-increase') && value < max) {
                value++;
            } else if(e.target.classList.contains('btn-decrease') && value > min) {
                value--;
            }
            input.value = value;
