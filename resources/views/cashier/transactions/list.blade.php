{{-- resources/views/cashier/transactions/list.blade.php --}}
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            <div id="product-list-wrapper">
                <div id="product-list" class="row row-cols-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 g-3">
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm sticky-top" style="top: 80px;">
                <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
                    ...
                    <div id="clear-cart-container">
                        @if(!$cartItems->isEmpty())
                            <form action="{{ route('cashier.cart.clear') }}" method="POST" onsubmit="return confirm('Clear cart?')">
                                @csrf
                ...
                <div class="card-body" id="mini-cart-body">
                    @include('cashier.transactions.partials.cart-items', ['cartItems' => $cartItems, 'cartTotal' => $cartTotal])
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timer = setTimeout(() => {
            fetch(`{{ route('cashier.products.list') }}?search=${encodeURIComponent(query)}`)
                .then(res => res.text())
                .then(html => {
                    const parser = new DOMParser();
                    const doc = parser.parseFromString(html, 'text/html');
                    const newContent = doc.querySelector('#product-list-wrapper').innerHTML;
                    productListWrapper.innerHTML = newContent;
            ...
            form.addEventListener('submit', function(e) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        updateClearButton(data.cartCount);
    ...
    // Update clear button visibility
    function updateClearButton(cartCount) {
